...
mypath <- ("/Users/theresavelden/Networks/StudentProjects/git/geomaps-project-files/geomaps/data/F1/temporal_clusters/")
...
scientometric.ani <- function()
{
    for (i in 1991:2009)
    {
	 ...
	 plotclr <- brewer.pal(nclr,"Spectral") # BuPu, YlOrRd 
	 plotclr <- plotclr[nclr:1] # reorder colors
#	 class <- classIntervals(plotvar, nclr, style="equal")
	 class <- classIntervals(symbol.size, nclr, style="equal")
